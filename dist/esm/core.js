...
var internalGa = function internalGa() {
  ...
  if (_testMode) return TestModeAPI.gtag.apply(TestModeAPI, arguments);
  if (_isNotBrowser) return false;
  if (!window.gtag) return warn('ReactGA.initialize must be called first or GoogleAnalytics should be loaded manually');
  ...
  require: function require(rawName, options, trackerName) {
    if (typeof gtag === 'function') {
      ...
      if (options) {
        ...
        if (_debug) {
          log("called gtag('require', '".concat(name, "', ").concat(JSON.stringify(options)));
        }
      } else {
        gtag(requireString, name);

        if (_debug) {
          log("called gtag('require', '".concat(name, "');"));
        }
      }
    }
  },
    ...
    if (typeof gtag === 'function') {
      if (typeof pluginName !== 'string') {
        ...
        if (actionType && payload) {
          ...
          if (_debug) {
            log("called gtag('".concat(command, "');"));
